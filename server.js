...
async function getState() {
  ...
  if (!Array.isArray(state.toners)) {
    state.toners = [];
  }

  if (!Array.isArray(state.movimentacoes)) {
    state.movimentacoes = [];
  }

  if (!Array.isArray(state.impressoras)) {
    state.impressoras = [];
  }
...
async function saveState(state) {
  if (
    !state ||
    !Array.isArray(state.toners) ||
    !Array.isArray(state.movimentacoes)
  ) {
...
async function findSession(req) {
  ...
  const { rows } = await pool.query(
    `SELECT technician_id, technician_name, expires_at
     FROM sessions
     WHERE token_hash=$1`,
    [tokenHash]
  );
...
app.post('/api/login', async (req, res, next) => {
  try {
    ...
    if (
      n.toLowerCase() === ADMIN_USER &&
      p === ADMIN_LOGIN_PASSWORD
    ) {
      user = {
        id: 'admin',
        name: ADMIN_USER
      ...
      if (
        row &&
        await bcrypt.compare(p, row.password_hash)
      ) {
        user = {
          id: row.id,
          name: row.name
...
// CADASTRO DE TÉCNICO
// =====================================================
// IMPORTANTE: não colocar "auth" aqui.
// O cadastro usa a senha de administrador.
app.post('/api/technicians', async (req, res, next) => {
  try {
    ...
    if (n.toLowerCase() === 'admin') {
      return res.status(400).json({
        error: 'Esse nome é reservado para o administrador.'
      });
    }
    ...
    if (e.code === '23505') {
      return res.status(409).json({
        error: 'Já existe um técnico com esse nome.'
      });
    }
...
app.get('/api/technicians', auth, async (req, res, next) => {
  try {
    if (req.user.id !== 'admin') {
      return res.status(403).json({
        error: 'Apenas o administrador pode visualizar os técnicos.'
      });
    }
...
app.delete('/api/technicians/:id', auth, async (req, res, next) => {
  try {
    if (req.user.id !== 'admin') {
      return res.status(403).json({
        error: 'Apenas o administrador pode excluir técnicos.'
      });
    }
...
app.listen(PORT, () => {
  console.log(
    `Servidor do estoque rodando na porta ${PORT}`
  );
});
